...
if (favList.length === 0)
  favCard.innerHTML =
    '<div style="margin: 10%; text-align: center; color: white; font-weight:bolder">Your Favourite Heroes meet here!!!<div>';
...
for (let id of favList) {
  fetch("https://superheroapi.com/api.php/1953802511419258/" + id + "/image")
    .then((res) => res.json())
    .then((data) => createCard(data, id)) // calling createCard function for each item in favList
...
favCard.onclick = function (event) {
  ...
  if (favList.length === 0)
    favCard.innerHTML =
      '<div style="margin: 10%; text-align: center; color: white;">Error! Favourites List Empty.<div>';
};
